...
/**
 * 
 * Miscellaneous utils for Oracle JDBC Redirect package
 * @author <a href="mailto:averemee@a2.solutions">Aleksei Veremeev</a>
 * 
 */
public class OraForwardUtils {

	public static final int POSTGRESQL = 0;
	public static final int MARIADB = 1;

	private final static Logger LOGGER = Logger.getLogger(OraForwardUtils.class.getName());
	private static final String PROPS_PATH = "/orajdbc-redirect-version.properties";
	...
	static {
		try (InputStream is = OraForwardUtils.class.getResourceAsStream(PROPS_PATH)) {
	...
	protected static SQLFeatureNotSupportedException sqlFeatureNotSupportedException(final String feature) {
		final String message = String.format("%s is not supported!", feature);
		return new SQLFeatureNotSupportedException(message);
	}

	protected static SQLSyntaxErrorException namedParameterNotFound(final String param, final String original, final String translated) {
		final String message;
		if (original.equals(translated)) {
			message = String.format("Parameter %s is not found for SQL statement:\n\t%s", param, original);
		} else {
			message = String.format("Parameter %s is not found for translated SQL statement:\n\t%s\nOriginal statement:\n\t%s",
					param, translated, original);
		}
